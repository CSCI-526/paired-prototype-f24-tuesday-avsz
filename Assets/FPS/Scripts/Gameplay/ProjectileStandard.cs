...
namespace Unity.FPS.Gameplay
{
    public class ProjectileStandard : ProjectileBase
    {
        ...
        void OnHit(Vector3 point, Vector3 normal, Collider collider)
        {
            ...
            {
            KeyHealth keyHealth = collider.GetComponent<KeyHealth>();
            if (keyHealth != null)
            {
                keyHealth.TakeDamage(1);
            }
            }
            // damage
            if (AreaOfDamage)
            ...
            else
            {
               ...
               // Apply an impulse to the object if it has a Rigidbody
            Rigidbody rb = collider.GetComponent<Rigidbody>();
            if (rb != null)
            {
                // Calculate the impulse direction and magnitude (you can customize the force amount)
                Vector3 impulseDirection = -normal; // Apply impulse in the direction of the surface normal
                float impulseForce = 100f;          // Adjust this value based on how strong you want the impact to be
                // Apply the impulse
